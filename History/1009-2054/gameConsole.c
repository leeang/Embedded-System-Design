...
ISR(INT2_vect) {
	...
	currentPixel = _BV(currentPixel) | frameBuffer[currentColumn][currentPage];
	frameBuffer[currentColumn][currentPage] = currentPixel;
	LCD_Tx(DATA, currentPixel);
	_delay_ms(255);
}
/* --------- /Interrupt Service Routine --------- */
...
int main(void) {
	...
	batteryLevel < BAT_MIN ? (BAT_LOW_LED(ON)) : (BAT_LOW_LED(OFF));
