...
byte LCD_Buffer[MAX_COLUMN][MAX_PAGE];
int currentRow = ROW;
...
ISR(INT1_vect) {
	...
	if (UP_BUTTON || DN_BUTTON || LT_BUTTON || RT_BUTTON) {
		if (keepPixel) {
			LCD_Buffer[currentColumn][currentPage] = currentRegister;
		}
		...
		currentRegister = _BV(currentPixel) | LCD_Buffer[currentColumn][currentPage];
		selectPage(currentPage);
		selectColumn(currentColumn);
		LCD_Tx(DATA, currentRegister);

		if (columnChange != 0 || pageChange != 0) {
			byte changeRegister = LCD_Buffer[currentColumn-columnChange][currentPage-pageChange];
